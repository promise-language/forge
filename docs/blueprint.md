...
- **Why those two are not the same case, when one rule would be cheaper to hold than two.** The hook has a reach the project owns and a gate an adopter can run instead: `core.hooksPath` is wired by `setup` and re-checked by `make` on every run, so the hook directory is live the moment the tools build, and a checkout provisioning never reaches still has `bin/verify` by hand. The agent guard has neither. Claude Code reads `.claude/settings.json` out of the checkout and nothing outside it points anywhere else, so wiring only provisioning writes is absent exactly in the checkout that is a first-class configuration rather than a half-finished install (workspace's `tool-contract.md`, A project stands alone) — and there is no by-hand equivalent, because a guard whose job is to stop a tool call before it happens has no form a person runs afterwards. Provisioning writes the file too, where it runs; committing it is what makes the guard live before it ever has.
